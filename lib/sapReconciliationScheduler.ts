...
//  - The "ready to write" list email runs once daily at a fixed time (currently 4:00 PM —
//    easy to retune below). It never trusts whatever the last hourly check happened
//    to find: it re-checks every outstanding vehicle against SAP directly first, then emails
//    the just-refreshed list — so nothing eligible between hourly checks gets missed. It never
//    writes to SAP — email only.
//  - The actual SAP write runs once daily, at a fixed time (currently 11:00 PM — easy to
//    retune below). Same principle: re-checks every outstanding vehicle against SAP directly
//    first (never relies on the 4:00 PM email's list or any previously saved eligibility),
//    emails the just-refreshed "writing now" list (distinct wording from the 4:00 PM email),
//    then writes every vehicle that's actually eligible right now (same chronological A→B→C
...
const EMAIL_HOUR = 16, EMAIL_MINUTE = 0; // "4:00 PM" — ready-to-write list email (no write)
const WRITE_HOUR = 23, WRITE_MINUTE = 0; // "11:00 PM" — fresh re-check, email, then write
...
// with the just-refreshed "ready to write" list. Email only — never writes to SAP.
...
// Fresh re-check → "writing now" email → write every vehicle actually eligible right now.
// Never relies on the 4:00 PM email's list or any previously saved eligibility — always
// re-checks SAP directly first (see runAutoWriteReadyVehicles).
