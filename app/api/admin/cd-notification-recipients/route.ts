...
export async function GET() {
  ...
  const data = await getCdNotificationRecipients();
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error("[admin/cd-notification-recipients] add failed:", e);
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    await prisma.$executeRaw`DELETE FROM "CdNotificationRecipient" WHERE "id" = ${id}`;
    return NextResponse.json({ success: true });
  } catch (e) {
    console.error("[admin/cd-notification-recipients] delete failed:", e);
