...
const typeConfig: Record<string, { icon: string; bg: string; color: string; label: string; labelBg: string; labelColor: string }> = {
  ...
  GATE_PASS_CANCELLED:      { icon: "🚫", bg: "#f3f4f6", color: "#6b7280", label: "Cancelled",        labelBg: "#f3f4f6", labelColor: "#6b7280" },
  GATE_PASS_RECEIVED:       { icon: "🚗", bg: "#f5f3ff", color: "#5b21b6", label: "Gate Out",         labelBg: "#f5f3ff", labelColor: "#5b21b6" },
  GATE_PASS_RESUBMITTED:    { icon: "🔄", bg: "#fff7ed", color: "#c2410c", label: "Resubmitted",      labelBg: "#fff7ed", labelColor: "#c2410c" },
  CASHIER_REVIEW_REQUIRED:  { icon: "💰", bg: "#fffbeb", color: "#b45309", label: "Payment Review",   labelBg: "#fffbeb", labelColor: "#b45309" },
...
function getNavPath(n: Notification): string {
  ...
  switch (n.type) {
    case "GATE_PASS_SUBMITTED":
    case "GATE_PASS_RESUBMITTED":    return `/gate-pass/approve/${id}`;
    case "GATE_PASS_APPROVED":
    case "GATE_PASS_REJECTED":       return `/gate-pass/${id}`;
    case "GATE_PASS_RECEIVED":       return `/recipient`;
    case "CASHIER_REVIEW_REQUIRED":  return `/gate-pass/${id}`;
